...
class Slash(commands.Cog):
    ...
    async def GetLyrics(self, ctx, 아티스트: str, 제목: str):
        ...
        if artist == None:
            ...
            if song == None:
              ...
              SongPage = 0
              embed = discord.Embed(title=f'{song.title}',description=f'{song.artist}\n\n{await ListToText(SongList,SongPage)}'.replace('[Verse 1]',"").replace('[Verse 2]',"").replace('[Verse 3]',"").replace('[Pre-Chorus]',"").replace('[Chorus]',"").replace('[Post-Chorus]',"").replace('[Bridge]',""), colour=0x3B88C3)
    ...
    async def 가사(self, ctx, 아티스트: str, 제목: str):
        embed = discord.Embed(title='🔎 검색 중',description=f"**`{아티스트}`** 님의 **`{제목}`**(을)를 찾는 중 이에요!", colour=0x3B88C3)
        message = await ctx.send(embed=embed)
        artist = await GetArtist(아티스트)
        if artist == None:
            embed = discord.Embed(title='🔄 아티스트 검색',description=f"**`{아티스트}`** 님을 찾을 수 없어요!", colour=0x3B88C3)
